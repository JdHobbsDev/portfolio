...
export async function GET() {
    try {
        ...
        const projectFolders = {
            ...
            'asteroid-studios-ticket': path.join(publicDir, 'projects', 'asteroid-ticket'),
            ...
            try {
                if (fs.existsSync(folderPath)) {
                    const files = fs.readdirSync(folderPath)
                        ...
                        .map(file => `/projects/${projectId.includes('minds-matter') ? 'minds-matter' :
                            projectId.includes('asteroid') ? 'asteroid-ticket' : 'utility-bot'}/${file}`);
